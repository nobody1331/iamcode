...
<head>
	<!-- Basic -->
	<meta charset="utf-8">
	<title>Murlimart - Welcome to eCommerce platform</title>
		...
		<div class="content offset-top-0" id="slider">
			...
			<div class="tp-banner-container">
				<div class="tp-banner">
					<ul>
						<?
						 $sqlslide = "select * from murlimart_sliderimg where status='1' order by disporder";
						 $resslide = mysqli_query($conn,$sqlslide);
						 while($rowslide = mysqli_fetch_assoc($resslide))
						 {
							 $sliderImage = "slider-images/".$rowslide['slider_img'];
						?>
						<!-- SLIDE -1 -->
						<li data-transition="fade" data-slotamount="1" data-masterspeed="1000" data-saveperformance="off"  data-title="Slide">
							<!-- MAIN IMAGE --> 
							<img src="<?=$sliderImage;?>"  alt="<?=$rowslide['slider_img']?>"  data-bgposition="center center" data-bgfit="cover" data-bgrepeat="no-repeat" > 
						</li>
						<? }?>						
			...
			<div class="content offset-top-0  fullwidth indent-col-none">
				<div class="container">
					<div class="row">
						...
						<?
						 $sqlslide = "select * from murlimart_mbanner where status='1' order by disporder";
						 $resslide = mysqli_query($conn,$sqlslide);
						 while($rowslide = mysqli_fetch_assoc($resslide))
						 {
							 $sliderImage = "slider-images/".$rowslide['slider_img'];
						?>
							<div class="col-md-3">
								<a href="<?=$rowslide['url_name']?>" class="banner zoom-in">
									<span class="figure">
										<img src="<?=$sliderImage?>" alt=""/>
										<span class="figcaption text-left">
											<span class="block-table">
												<span class="block-table-cell">
													<span class="block font-size82 text-uppercase font-medium"><?=$rowslide['offertext-1']?></span>
													<span class="block text-uppercase font-size26"><?=$rowslide['offertext-2']?></span>
													<em class="btn btn--border btn--xl top-indent-md">Shop now! <span class="icon icon-navigate_next"></span></em>
												</span>
											</span>
										</span>
									</span>
...
<?
 $sqlcat1 = "select * from murlimart_category where status='1'";
 $rescat1 = mysqli_query($conn,$sqlcat1);
 while($rowcat1 = mysqli_fetch_assoc($rescat1))
 {
?>
        <div class="row">
            <div class="col-lg-12">
                <h1 style="text-align:center; margin:15px; margin-bottom:40px" class="text-center text-uppercase title-under"><?=$rowcat1['catname'];?> </h1>
            </div>
        </div>
        <!-- carousel -->
        <div class="row">
		...
		<?
        $i = 1;
        $sqlprod="SELECT * from murlimart_product where status='1' and catid_reff='".$rowcat1['catid']."' order by prodid DESC LIMIT 8";
        $resprod = mysqli_query($conn,$sqlprod);
        while($rowprod=mysqli_fetch_assoc($resprod))
        {
            //min prod price
            $sqlprice1="select min(mrp) minmrp , min(sellprice) minsp from murlimart_product_size where prodid_reff='".$rowprod['prodid']."' and stock>0";
            $resprice1 = mysqli_query($conn,$sqlprice1);
            $rowprice1=mysqli_fetch_assoc($resprice1);
            $minmrp = $rowprice1['minmrp'];
            $minsp  = $rowprice1['minsp'];
            
            $sqlprice2 = "select * from murlimart_product_size where prodid_reff='".$rowprod['prodid']."' and stock>0";
            $resprice2 = mysqli_query($conn,$sqlprice2);
            $rowprice2 = mysqli_fetch_assoc($resprice2);
            
    
            $uploaddir="products-images/".$rowprod['image1'];
        ?>
            ...
            <div class="col-lg-3">
                ...
                <div class="product">
                    <div class="product__inside">
                        <!-- product image -->
                        <div class="product__inside__image" align="center">
                            <a href="product-details.php?prodid=<?=$rowprod['prodid'];?>&name=<?=$rowprod['prodname'];?>"> <img src="<?=$uploaddir?>" alt="" style="width:250px;height:250px;"> </a> 
                        </div>
                        <!-- /product image --> 											
                        <!-- product name -->
                        <div class="product__inside__name" style="height:45px;">
                            <h2><a href="product-details.php?prodid=<?=$rowprod['prodid'];?>&name=<?=$rowprod['prodname'];?>"><?=substr($rowprod['prodname'],0,70);?></a></h2>
                        </div>
                        <!-- /product name --> 
                        <!-- product price
                        <div class="product__inside__price price-box" style="margin:0 0 0.1em 0"><span class="price-box__old">&#8377;<?=$minmrp?> - &#8377;<?=$maxmrp?></span></div> -->
                        <div class="product__inside__price price-box"><span class="price-box__old">&#8377;<?=$minmrp?></span> - &#8377;<?=$minsp?></div>
    
                        
                        <!-- /product price --> 
    
                        <div class="">
                        <div class="product__inside__info__btns"> <a href="AddToCart.php?prodszid=<?=$rowprice2['psizeid'];?>&prodid=<?=$rowprod['prodid'];?>" class="btn btn--ys btn--xl"><span class="icon icon-shopping_basket"></span> Add to cart</a>
                                </div>
        ...
        <?
            if($i%4=='0')
            {?>
            </div><div class="row">	<div class="col-lg-12" style="margin-top:25px;"></div></div>
            <div class="row">
  ...
  <? }?>
	...
	<!-- /carousel -->
			...
			<div class="container-fluid box-baners content">
				<div class="row">					
					<div class="col-xs-12">
						<a href="products.php" class="banner banner-bg image-bg zoom-in" data-image="images/custom/banner-23.jpg">
							<span class="figure">
				...
				<div class="container">
					<div class="row">
						...
						<div class="col-xs-12 col-sm-6 col-lg-6 col-xl-4">
							<h2 class="text-left text-uppercase title-under hidden-xs">Services</h2>
                            <h2 class="text-center text-uppercase title-under visible-xs">Services</h2>
							
							<div class="banner-icon" onclick="window.location.href = '#'">
								<div class="banner-icon__icon hidden-xs"><span class="icon icon-send"></span></div>
							...
							<div class="banner-icon" onclick="window.location.href = '#'">
								<div class="banner-icon__icon hidden-xs"><span class="icon icon-loop"></span></div>
								<div class="banner-icon__text">
									<h3>Order Return</h3>
									<p>
							...
							<div class="banner-icon" onclick="window.location.href = '#'">
								<div class="banner-icon__icon hidden-xs"><span class="icon icon-headset_mic"></span></div>
								<div class="banner-icon__text">
			...
			<!-- brands-carousel 
			<div class="content container-fluid fill-bg2 fill-bg">
				<? //include("includes/allbrand.php");?>
			</div>-->
			<!-- /brands-carousel -->	
					...
					<div class="row">
						<div class="banner-carousel-1">
							...
							<div class="col-md-3 col-sm-6 col-xs-12">
								<!-- banner -->
								<a href="products.php" class="banner banner-md bg-pink  zoom-in">
									<span class="figure">
							...
							<!--  -->
							<div class="col-md-3 col-sm-6 col-xs-12">
								<a href="#" class="banner banner-md bg-light-blue zoom-in">
									<span class="figure">
										<span class="figcaption text-center">
											<span class="block-table">
												<span class="block-table-cell">
		...
		<script src="js/js-index-07.js"></script>					
	</body>
</html>