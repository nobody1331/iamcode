...
include('header.php');
$catid=$_GET['catid'];
if($langquery==''){$language='All Language';}
?>
      <!-- Slider Start -->
      <section id="home" class="iq-main-slider p-0">
         <div id="home-slider" class="slider m-0 p-0">
<?php
   $queryw="SELECT *  FROM `content_list` where  `status`=0 $langquery order by avgrate desc";
	$resultxxw=mysqli_query($as,$queryw);
	for($m=0;$m<=mysqli_num_rows($resultxxw)-1;$m++)
	{
	$rown1=mysqli_fetch_array($resultxxw);
	$season=$rown1['season'];
	 $contentname=$rown1['name'];
	$banner=$rown1['banner'];
	$plan=$rown1['plan'];	 
	$contentid=$rown1['id'];	 
	$desc=$rown1['desc'];	 
	$age=$rown1['age'];	 
	$lang=$rown1['lang'];	 
	$trailer=$rown1['trailer'];	 
	$genres=$rown1['generes'];	 
	$noseason=$rown1['season'];	 

	$queryws="SELECT *  FROM `videos` where `cid`  = '$contentid' order by id desc ";
	$resultxxws=mysqli_query($as,$queryws);
	$fdssfx=mysqli_fetch_array($resultxxws);
	$vid=$fdssfx['id'];
	$dur=$fdssfx['dur'];
	$video=$fdssfx['video'];
	$url="play_vid.php?vid=$vid";
	if($noseason==0)
	{
	$season='';
	$rown=$dur." m.";
	...
	else{$rown=" Season ".$noseason;}

	?>
            <div class="slide slick-bg s-bg-2" style="background-image: url('images/<?php echo $banner; ?>'); height: 100%; width: 100%; ">
                     ...
                     <div class="row align-items-center  h-100">
                        <div class="col-xl-6 col-lg-12 col-md-12">
                           <a href="javascript:void(0);">
							 ...
							 Latest on
                                 <img src="images/logo.png" class="c-logo" alt="Maginato">
                              </div>
                           </a>
                           <h1 class="slider-text big-title title text-uppercase" data-animation-in="fadeInLeft"
                              data-delay-in="0.6"><?php echo $contentname; ?></h1>
                           <div class="d-flex align-items-center" data-animation-in="fadeInUp" data-delay-in="1">
                              <span class="badge badge-secondary p-2"><?php echo $age; ?>+</span>
                              <span class="ml-3"><?php echo $rown; ?> </span>
                           </div>
                           <p data-animation-in="fadeInUp" data-delay-in="1.2"><?php echo $desc; ?>
                           </p>
                           <div class="d-flex align-items-center r-mb-23" data-animation-in="fadeInUp" data-delay-in="1.2">
                              <a href="<?php echo $url; ?>" class="btn btn-hover"><i class="fa fa-play mr-2"
                                 aria-hidden="true"></i>Play Now</a>
                              <!--a href="#" class="btn btn-link">More details</a-->
                           </div>
						  <p> <div class="text-primary title">Genres: <span class="text-body">
												<?php echo $genres; ?></span>
                             </p>                   </div>
                        </div>
                     </div>
                     <!--div class="trailor-video">
                        <a href="video/<?php echo $video; ?>" class="video-open playbtn">
...
<?php include('favourite.php'); ?>
  <?php include('latest.php'); ?>      
   <?php include('wlater.php'); 
$query="SELECT * FROM `content_list` where  `status`=0  $langquery order by viewtime desc";
	$resultxx=mysqli_query($as,$query);
	$fdssf=mysqli_fetch_array($resultxx);
	$banner=$fdssf['banner'];	
	$age=$fdssf['age'];
	$contentname=$fdssf['name'];
	$image=$fdssf['banner'];
	$plan=$fdssf['plan'];
	$noseason=$fdssf['season'];
	$cid=$fdssf['id'];
	if($plan==0){$plannm="Basic";}
	elseif($plan==1){$plannm="Standard";}
	elseif($plan==2){$plannm="Premiun";}
	elseif($plan==3){$plannm="Platinum";}
	 	elseif($plan==4){$plannm="Exclusive";}

	$queryws="SELECT *  FROM `videos` where `cid`  = '$cid' order by id desc";
	$resultxxws=mysqli_query($as,$queryws);
	$fdssfx=mysqli_fetch_array($resultxxws);
	$vid=$fdssfx['id'];
	$dur=$fdssfx['dur'];
...
?>  
    ...
    <section id="parallex" class="parallax-window" style="background-image: url('images/<?php echo $image; ?>'); height: 100%; width: 100%; ">
            <div class="container-fluid h-100">
               <div class="row align-items-center justify-content-center h-100 parallaxt-details">
				...
				<div class="col-lg-4 r-mb-23">
                     <div class="text-left">
                        <a href="javascript:void(0);">
                        <img src="images/parallax/parallax-logo.png" width='25%' class="img-fluid" alt="bailey">
                        </a>
                        <div class="parallax-ratting d-flex align-items-center mt-3 mb-3">
   <?php include('avgrate.php'); ?>      
                          
						  <ul>
						  <?php echo $avgstar; ?>
                           
                           </ul>
                           <span class="text-white ml-3"><?php echo $avg; ?></span>
                        </div>
						<h3><?php echo $contentname; ?></h3>
                        <div class="movie-time d-flex align-items-center mb-3">
						 
                           <div class="badge badge-secondary mr-3"><?php echo $age; ?>+</div>
                           <h6 class="text-white"><?php echo $rown; ?></h6>
                        </div>
                        <p><?php echo $desc; ?></p>
                        <div class="parallax-buttons">
                           <a href="<?php echo $url; ?>" class="btn btn-hover">Play Now</a>
                        </div>
                     </div>
                  </div>
                  <div class="col-lg-8">
                     <div class="parallax-img">
                        <a href="<?php echo $url; ?>">
                        	<img src="images/<?php echo $banner; ?>" class="img-fluid w-100" alt="bailey">
                        </a>
                     </div>
                  </div>
               </div>
